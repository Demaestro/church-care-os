...
// ── Access level helpers (duplicated from permissions.js for client safety) ──
const ACCESS_LEVELS = ["none", "read", "worker", "lead", "full"];
function lvl(level) {
  const i = ACCESS_LEVELS.indexOf(String(level || "none"));
  return i === -1 ? 0 : i;
}
function canSeeModule(permissions, module, minLevel = "read") {
  return lvl(permissions?.[module] ?? "none") >= lvl(minLevel);
}

// ── Nav section definitions ──────────────────────────────────────────────────
// Each section declares which module + minimum access level is required.
// Items can also declare per-item overrides (module + minLevel on the item).
const NAV_SECTIONS = [
  {
    label: "People",
    module: "membership",
  ...
  },
  {
    ...
    minLevel: "worker",
    items: [
      { href: "/leader",       label: "Ministry Board", icon: "M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" },
      ...
      { href: "/discipleship", label: "Discipleship",   icon: "M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253", module: "discipleship", minLevel: "worker" },
    ],
  },
  {
    label: "Worship",
    ...
    items: [
      ...
      { href: "/calendar",   label: "Ministry Calendar", icon: "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" },
      { href: "/schedule",   label: "Schedule",          icon: "M19 4H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V6a2 2 0 00-2-2zM16 2v4M8 2v4M3 10h18" },
    ],
  },
  {
    label: "Finance",
  ...
  },
  {
    ...
    minLevel: "lead",
    items: [
      { href: "/pulse",     label: "Pulse Dashboard", icon: "M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" },
      { href: "/analytics", label: "Analytics",       icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z", module: "analytics", minLevel: "read" },
      { href: "/regions",   label: "Branches",        icon: "M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7", module: "admin", minLevel: "read" },
    ],
  },
  {
    label: "Admin",
    module: null, // always visible (items have individual guards)
    minLevel: "none",
    items: [
      { href: "/assets",    label: "Asset Register", icon: "M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4", module: "assets",  minLevel: "read" },
      ...
      { href: "/settings",  label: "Settings",       icon: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z", module: "admin", minLevel: "read" },
...
export default function LeftNav({ user, unreadCount = 0, permissions = {} }) {
  ...
  // Filter sections — hide entire section if user lacks module access
  const visibleSections = NAV_SECTIONS.filter(section => {
    if (!section.module) return true;
    return canSeeModule(permissions, section.module, section.minLevel);
  }).map(section => ({
    ...section,
    // Filter individual items that have their own module guard
    items: section.items.filter(item => {
      if (!item.module) return true;
      return canSeeModule(permissions, item.module, item.minLevel);
    }),
